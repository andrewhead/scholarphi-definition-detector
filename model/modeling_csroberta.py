...
class CSRoberta(BertPreTrainedModel):
    config_class = RobertaConfig
    # pretrained_model_archive_map = ROBERTA_PRETRAINED_MODEL_ARCHIVE_LIST
    base_model_prefix = "csroberta"

    def __init__(self, config, args, intent_label_lst, slot_label_lst, pos_label_lst):
        super(CSRoberta, self).__init__(config)
        self.args = args
    ...
    def forward(self, input_ids, attention_mask, token_type_ids, intent_label_ids, slot_labels_ids,
                pos_label_ids=None,
		np_label_ids=None, vp_label_ids=None, entity_label_ids=None, acronym_label_ids=None
		):
        ...
        if self.args.use_np:
            sequence_output = torch.cat([sequence_output, np_label_ids.unsqueeze(2)], dim=2)
        if self.args.use_vp:
            sequence_output = torch.cat([sequence_output, vp_label_ids.unsqueeze(2)], dim=2)
        if self.args.use_entity:
            sequence_output = torch.cat([sequence_output, entity_label_ids.unsqueeze(2)], dim=2)
        if self.args.use_acronym:
            sequence_output = torch.cat([sequence_output, acronym_label_ids.unsqueeze(2)], dim=2)
