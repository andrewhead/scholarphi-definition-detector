...
class JointRoberta(BertPreTrainedModel):
    ...
    def __init__(self, config, args, intent_label_dict, slot_label_dict, pos_label_lst, tasks):
    ...
    def forward(self, input_ids, attention_mask, token_type_ids, intent_label_ids, slot_label_ids,
        pos_label_ids=None, np_label_ids=None, vp_label_ids=None, entity_label_ids=None, acronym_label_ids=None
		):

        outputs = self.roberta(input_ids, attention_mask=attention_mask,
                            token_type_ids=token_type_ids)  # sequence_output, pooled_output, (hidden_states), (attentions)
        sequence_output = outputs[0] # [ batch x length x 768]
        pooled_output = outputs[1]  # [batch x 768 ]

        # print('\n\n\n')
        # print('CHECK IF CUDA')
        # print(input_ids.is_cuda)
        # print(sequence_output.is_cuda)
        # print(pooled_output.is_cuda)
        # print('\n\n\n')
        ...
        if self.args.use_np:
            sequence_output = torch.cat([sequence_output, np_label_ids.float().unsqueeze(2)], dim=2)
        if self.args.use_vp:
            sequence_output = torch.cat([sequence_output, vp_label_ids.float().unsqueeze(2)], dim=2)
        if self.args.use_entity:
            sequence_output = torch.cat([sequence_output, entity_label_ids.float().unsqueeze(2)], dim=2)
        if self.args.use_acronym:
            sequence_output = torch.cat([sequence_output, acronym_label_ids.float().unsqueeze(2)], dim=2)


        if self.args.use_pos or self.args.use_np or self.args.use_vp or self.args.use_entity or self.args.use_acronym:
            pooled_output = self.custom_pooler(sequence_output)
            # [ batch x length x 768+11] x [ batch x 768+11 x 768+11] => [batch x 768+11]
        ...
        outputs = (total_loss_dict,) + outputs
